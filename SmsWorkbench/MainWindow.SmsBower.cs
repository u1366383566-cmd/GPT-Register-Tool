...
namespace SmsWorkbench
{
    ...
    {
        private Task<string> ShowOneClickSmsProviderDialogAsync()
        {
            ...
            combo.Items.Add(new ComboBoxItem { Content = "5SIM（默认）", Tag = "5sim" });
            combo.Items.Add(new ComboBoxItem { Content = "SMSBower", Tag = "smsbower" });
            ...
            ok.Click += (_, _) =>
            {
                selected = (combo.SelectedItem as ComboBoxItem)?.Tag as string ?? "";
        ...
        private async Task<bool> ShowFiveSimOneClickDialogAsync()
        {
            string apiKey = ResolveFiveSimApiKey(settingsService.GetString("phone_reuse.5sim.api_key"));
            if (string.IsNullOrWhiteSpace(apiKey))
            {
                ShowThemedInfoDialog("5SIM 未配置", "请先在设置的手机接码分类中填写 5SIM API Token。");
                return false;
            }

            string endpoint = FirstNonEmpty(settingsService.GetString("phone_reuse.5sim.endpoint"), FiveSimCatalogClient.DefaultEndpoint);
            string product = FirstNonEmpty(settingsService.GetString("phone_reuse.5sim.product"), FiveSimCatalogClient.OpenAiProduct);
            IReadOnlyList<FiveSimCountryChoice> countries;
            string balance = "--";
            try
            {
                System.Windows.Input.Mouse.OverrideCursor = System.Windows.Input.Cursors.Wait;
                countries = await FiveSimCatalogClient.LoadOpenAiCatalogAsync(httpClient, apiKey, endpoint, product);
                try
                {
                    balance = await FiveSimCatalogClient.LoadBalanceAsync(httpClient, apiKey, endpoint);
                }
                catch (Exception balanceError)
                {
                    logger?.Warning(balanceError, "Failed to load 5sim balance");
                }
            }
            catch (Exception exc)
            {
                logger?.Error(exc, "Failed to load 5sim OpenAI catalog");
                ShowThemedInfoDialog("5SIM 加载失败", "无法读取 OpenAI 号码地区和运营商：" + exc.Message);
                return false;
            }
            finally
            {
            ...
            {
                ShowThemedInfoDialog("暂无号码", $"5SIM 当前没有可用的 OpenAI（{product}）号码。");
                return false;
            }

            string savedCountry = FirstNonEmpty(settingsService.GetString("phone_reuse.5sim.country"), "ghana");
            string savedOperator = FirstNonEmpty(settingsService.GetString("phone_reuse.5sim.operator"), "any");
            var selectedCountry = countries.FirstOrDefault(item => string.Equals(item.Id, savedCountry, StringComparison.OrdinalIgnoreCase))
                ?? countries[0];
            var selectedOperator = selectedCountry.Operators.FirstOrDefault(item => string.Equals(item.Id, savedOperator, StringComparison.OrdinalIgnoreCase))
            ...
            var headingPanel = new StackPanel { Margin = new Thickness(0, 0, 0, 18) };
            ...
            var productPanel = CreateSmsBowerDialogRow("服务商", out ContentControl productHost);
            productHost.Content = new TextBlock
            {
                Text = "OpenAI (ChatGPT) · " + product,
                FontSize = 14,
                FontWeight = FontWeights.SemiBold,
                Foreground = (Brush)FindResource("TextMain"),
            ...
            var operatorPanel = CreateSmsBowerDialogRow("运营商", out ContentControl operatorHost);
            var operatorBox = new ComboBox
            {
                ItemsSource = selectedCountry.Operators,
                DisplayMemberPath = nameof(FiveSimOperatorChoice.DisplayName),
                SelectedItem = selectedOperator,
                MaxDropDownHeight = 260,
                MinHeight = 36,
                Padding = new Thickness(8, 4, 8, 4)
            };
            operatorHost.Content = operatorBox;
            Grid.SetRow(operatorPanel, 3);
            root.Children.Add(operatorPanel);
            ...
            {
                if (operatorBox.SelectedItem is FiveSimOperatorChoice selected)
                {
                    inventory.Text = $"当前库存 {selected.Count} 个，价格 ${selected.Price.ToString("0.########", CultureInfo.InvariantCulture)} / 个";
                }
                else
                {
                    inventory.Text = "";
            ...
            {
                if (countryBox.SelectedItem is not FiveSimCountryChoice country) return;
                operatorBox.ItemsSource = country.Operators;
                operatorBox.SelectedItem = country.Operators[0];
                RefreshInventory();
            };
            operatorBox.SelectionChanged += (_, _) => RefreshInventory();
            RefreshInventory();
            ...
            if (dialog.ShowDialog() != true
                || countryBox.SelectedItem is not FiveSimCountryChoice chosenCountry
                || operatorBox.SelectedItem is not FiveSimOperatorChoice chosenOperator)
            {
                return false;
            }

            settingsService.UpdateConfig(configRoot =>
            {
                JsonObject phoneReuse = GetOrCreateSection(configRoot, "phone_reuse");
                JsonObject fiveSim = GetOrCreateSection(phoneReuse, "5sim");
                fiveSim["product"] = product;
                fiveSim["country"] = chosenCountry.Id;
                fiveSim["operator"] = chosenOperator.Id;
                if (chosenOperator.Price > 0)
                {
                    fiveSim["max_price"] = chosenOperator.Price.ToString("0.########", CultureInfo.InvariantCulture);
                }
                phoneReuse["source"] = "5sim";
            });
            return true;
        }
        ...
        private async Task<bool> ShowSmsBowerOneClickDialogAsync()
        {
            ...
            settingsService.UpdateConfig(root =>
            {
                ...
                JsonObject smsBower = GetOrCreateSection(phoneReuse, "smsbower");
        ...
        private static string ResolveFiveSimApiKey(string configured)
        {
            string value = (configured ?? "").Trim();
            if (value.Length == 0 || value == "$5SIM_API_KEY" || value == "YOUR_5SIM_API_KEY")
            {
                return (Environment.GetEnvironmentVariable("5SIM_API_KEY") ?? "").Trim();
            }
            return value;
        }
